...
export default function Home() {
  ...
  const fetchPosts = async () => {
    try {
      const res = await fetch("http://localhost:1212/posts", {
        headers: {
  ...
  const postLike = async (postId: string) => {
    const res = await fetch(`http://localhost:1212/posts/toggle-like/${postId}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-type": "application/json",
  ...
  const followUser = async (followedUserId: string) => {
    const res = await fetch(`http://localhost:1212/users/follow-toggle/${followedUserId}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-type": "application/json",
...
>
  Log Out
      ...
      <div className="space-y-6">
        {posts.length > 0 ? (
          posts.map((post: any) => (
            <div key={post._id} className="bg-white border rounded-md shadow-sm">
  ...
  {post.user.followers.includes(myId!) ? (
    ...
    >
      Follow
              ...
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-4">
                  <button onClick={() => postLike(post._id)}>
                    {post.likes.includes(myId) ? <Heart fill="red" color="red" /> : <Heart />}
                  </button>
                 <MessageCircle onClick={() => push(`/comments/${post._id}`)} />
              ...
              <div className="px-4 text-sm font-semibold">{post.likes.length} likes</div>
